...
IMAGES="${1:-conda-rolling}"
ARCHS="${2:-amd64}"

BASE_IMAGES="conda-rolling conda-dev conda-last-release"
EXTRA_IMAGES="conda-experimental conda-bleeding-edge"
...
# importance then.
ORDER="conda-rolling conda-last-release conda-bleeding-edge conda-experimental conda-dev"
for image in $(printf "%s\n" $ORDER | tac); do
    if echo "$IMAGES" | grep -qw $image; then
